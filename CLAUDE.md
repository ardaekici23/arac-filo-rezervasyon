...
Backend only exists today. `frontend/` is an intentional placeholder (see `frontend/README.md`) — React + Vite has not been set up yet.
...
backend/src/main/java/com/aracfilo/
  ...
  common/        cross-cutting: NotFoundException, BusinessRuleException, ApiErrorResponse, GlobalExceptionHandler
```
...
- `Reservation`'s constructor enforces `bitisTarihi.isAfter(baslangicTarihi)` via `BusinessRuleException`. `DateRange.overlaps()` (in `reservation/DateRange.java`) is a ready-made helper for the reservation overlap/conflict check — it exists but is **not yet wired into `ReservationService.create()`** (see the `TODO` there). This is the most important unimplemented business rule in the project.
...
- `ReservationController` has a `PATCH /api/rezervasyonlar/{id}/durum` endpoint for status transitions (`ReservationStatusRequest`), but nothing currently drives automatic transitions (e.g. PLANLANDI → TAMAMLANDI over time) — status changes are manual/API-driven only.
- There is no cancel-specific endpoint; cancellation is expected to go through the same `durum` PATCH endpoint (setting `IPTAL`).
- No `/api/araclar/musait` (available-vehicles-by-date-range) endpoint exists yet.
