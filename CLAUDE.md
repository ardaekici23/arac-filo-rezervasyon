...
- `Vehicle` grew two nullable columns beyond the original `V1` schema, each via its own additive migration: `foto_url` (`V5`, a plain TEXT holding either a pasted URL or a client-generated base64 data URI — no file-upload endpoint/storage exists) and `yil` (`V6`, integer). Both follow the same pattern: additive column, DTO field added to `VehicleRequest`/`VehicleResponse`, never a rename of the stable `markaModel`/`plaka`/`tur`/`durum` fields.
- The frontend's vehicle-add/edit form (`VehicleFormModal.jsx`) offers Marka/Model as two dependent dropdowns (`frontend/src/domain/vehicleCatalog.js` — a hardcoded brand→model catalog ported from the design) purely for UX; on save they're joined back into the single `markaModel` string the backend has always expected, so this required zero backend change. When editing, the stored `markaModel` string is split back into marka/model by matching against the catalog (`markaModelAyristir`) — an unmatched legacy value falls back to an empty selection rather than being lost.
